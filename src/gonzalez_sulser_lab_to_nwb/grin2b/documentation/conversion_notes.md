...
**NeuroConv version:** the HERD ontology support (`neuroconv.tools.ontology`) is not in a released NeuroConv yet; the pipeline currently runs against a local NeuroConv checkout that merges the `add-brain-region-herd-ontology` → `add-strain-herd-ontologies` → `add-anatomy-herd-ontology` branches onto `main`. That also brings NeuroConv `main`'s dict-based ecephys metadata (#1934, #1941), so `TainiRecordingInterface.get_metadata()` now emits a top-level `Devices` registry and `Ecephys["ElectrodeGroups"]` / `Ecephys["ElectricalSeries"]` keyed by metadata key (groups link to the device via `device_metadata_key`), and the interface passes `metadata_key=` instead of the deprecated `es_key=`. Pin `neuroconv` in `pyproject.toml` once the HERD work is released.
...
- **Ontology annotation (HERD)** — `grin2b_general_metadata.yaml` carries an `ontology` block in NeuroConv's file-wide format (`ontology.species` / `ontology.strain` / `ontology.brain_regions`, each keyed by the exact value string it annotates, each term an explicit `{id, uri}`). NeuroConv's `run_conversion` writes these as HERD references under `/general/external_resources` automatically — no call in `convert_session.py`. Terms are stated explicitly, not inferred with `infer_*_ontology_metadata`: the per-channel location labels (`S1_Tr`, `M2_Fra`, ...) are lab acronyms that NeuroConv's rat (UBERON fallback) vocabulary does not recognize, and NeuroConv's generic `Long-Evans` strain term is the Charles River `Crl:LE` RRID (`RGD_2308852`), whereas these animals are the MCW line `LE-Grin2bem1Mcwi` (`RRID:RGD_14394515`, used for `Subject.strain = "Long-Evans"`). A GRIN2B_129 stub produces 12 references: species (`NCBITaxon:10116`), strain, 8 electrode-table `location` values, and the 2 `ElectrodeGroup.location` values (`cortex` → `UBERON:0000956`, `trapezius muscle` → `UBERON:0002380`). This replaced the earlier top-level `BrainRegions` key, which newer NeuroConv no longer reads.
...
8. **Move `Device.manufacturer` to a `DeviceModel`** — pynwb 4.0 deprecates `Device.manufacturer` (`DeprecationWarning` on every conversion: "use DeviceModel.manufacturer and link to that DeviceModel from this Device"). In `TainiRecordingInterface.get_metadata()`, move `manufacturer: "TainiTec"` (and the model, e.g. the TainiTec headstage / NeuroNexus H16-Rat EEG16 grid) into a top-level `metadata["DeviceModels"]` entry and link it from the `TainiTecWirelessEEG` device via `device_model_metadata_key`.
